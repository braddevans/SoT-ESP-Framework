...
class BarrelsReading:
    ...
    def read_barrels(self):
        # List with length == 1, where first element is a 
        # Boolean - should we scan barrels or not (holding crate or not)
        if not self._barrels_should_update[0]:  
            return                       

        # If we don't have barrels to read from
        if not self._barrels_shared:                
            return

        # _barrels_shared is a dict, where:
        # Key == address for Barrel.StorageContainerComponent
        # Value == content of a barrel in a string format
        actors = list(self._barrels_shared.keys())  
        
        for actor in actors:                   
            ret_value = ""

            container_nodes = struct.unpack("<Qii", self.rm.read_bytes(
            ...
            if container_nodes[1] > 0:
                for i in range(0, container_nodes[1]):
                    ...
                    ItemDesc = self.rm.read_ptr(node + OFFSETS.get("StorageContainerNode.ItemDesc"))
                    actor_id = self.rm.read_int(ItemDesc + OFFSETS.get('Actor.actorId'))

                    if actor_id not in self.items_map:
                        gname = self.rm.read_gname(actor_id)
                        self.items_map[actor_id] = BarrelsReading.get_item_short_name(gname)
                
                    item_name = self.items_map[actor_id]
                    item_count = self.rm.read_int(node + OFFSETS.get("StorageContainerNode.NumItems"))
                    ret_value += f'\n- {item_count} {item_name}'

            if actor in self._barrels_shared:
                self._barrels_shared[actor] = ret_value
